...
for i in range(NUM_CLUSTERS):
    SAMPLE_POINT.append(random.randint(1,45780))
...
def initialize_centroids():
    if NUM_CLUSTERS == 1:
        centroids.append(Centroid(SAMPLES[SAMPLE_POINT[0]][0], SAMPLES[SAMPLE_POINT[0]][1], SAMPLES[SAMPLE_POINT[0]][2]))
    for i in range(NUM_CLUSTERS):
    ...
    for i in range(NUM_CLUSTERS):
        for j in range(TOTAL_DATA):
            if data[j].get_cluster() == i:
                distance = get_distance(data[j].get_x(),data[j].get_y(),data[j].get_z(),centroids[i].get_x(),centroids[i].get_y(), centroids[i].get_z())
